$(document).ready(function () {
                ...
                for (let product of productArr) {
                    ...
                    <div class="col-12 col-md-4 col-lg-3 mb-5" >
                        <a class="product-item" href="product.php?id=${product.id_product}">
                            ...
                            <strong class="product-price">&dollar;${product.price_product}</strong>
    ...
    function calculateTotalPrice(isAddedMore) {
        ...
        $('#subtotalValue').html('&dollar;' + subtotal.toFixed(2));
        $('#totalValue').html('&dollar;' + total.toFixed(2));
    }
    ...
    $(document).on('click', '.decreaseButton', function () {
        let inputQuant = $(this).siblings('.inputQuant');
        let currentQuantVal = parseInt(inputQuant.val());

        if (currentQuantVal > 1) {
            currentQuantVal--;
            inputQuant.val(currentQuantVal);

            let totalPrice = parseFloat($(this).closest('.cart-item-container').find('.totalValue').text().replace('$', ''));
            let priceMobile = parseFloat($(this).data("price"));
            let totalValueMobile = calculatePrice(priceMobile, currentQuantVal, "dec", totalPrice);
            $(this).closest('.cart-item-container').find('.totalValue').html('&dollar;' + totalValueMobile.toFixed(2));

            calculateTotalPrice(false);

            let productID = $(this).data("product-id");
            updateCartAjax(productID, currentQuantVal, totalValueMobile);
        }
    });
    ...
    $(document).on('click', '.increaseButton', function () {
        ...
        if (currentQuantVal < maxValue) {
            ...
            let priceMobile = parseFloat($(this).data("price"));
            let totalValueMobile = calculatePrice(priceMobile, currentQuantVal, "inc");
            $(this).closest('.cart-item-container').find('.totalValue').html('&dollar;' + totalValueMobile.toFixed(2));
